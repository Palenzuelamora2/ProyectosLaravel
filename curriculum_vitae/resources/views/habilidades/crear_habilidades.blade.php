<x-app-layout>
    <x-slot name="header">
        <h2 class="font-semibold text-xl text-gray-800 leading-tight">
            {{ __('Agregar Habilidad') }}
        </h2>
    </x-slot>
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            <form method="POST" action="{{ route('habilidad.store') }}" enctype="multipart/form-data">
                @csrf
                <!-- Empresa -->
                <div class="mt-4">
                    <x-input-label for="nombre_habilidad" :value="__('Habilidad')" />
                    <x-text-input id="nombre_habilidad" name="nombre_habilidad" type="text" class="mt-1 block w-full" />
                </div>
